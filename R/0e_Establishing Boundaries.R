...
if(file.exists("Output Files/RumBoundary.rds")){
  ...
  Boundary <- RumBoundary@polygons[[1]]@Polygons[[1]]@coords %>% as.data.frame()
  
  Boundary %<>% rename(Easting = 1, Northing = 2)
  ...
  write.csv(Boundary, file = "Output Files/RumBoundary.csv", row.names = F)
